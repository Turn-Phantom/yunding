...
public class InterAddressUtils {
    ...
    public static boolean isEnablePort(Map params, String serverName){
        if(null == params){
            LOGGER.error("启动失败：未配置"+ serverName +"地址信息");
            return false;
        }
        String host = (String)params.get("host");
        String port = (String)params.get("port");
        if(StringUtils.isEmpty(host)){
            LOGGER.error("启动失败：未配置"+ serverName +"主机地址");
            return false;
        }
        if(StringUtils.isEmpty(port)){
            LOGGER.error("启动失败：未配置"+ serverName +"主机端口");
            return false;
        }
        boolean enablePort = InterAddressUtils.isEnablePort(host, Integer.valueOf(port));
        if(!enablePort){
            LOGGER.error("启动失败：无法访问"+ serverName);
            return false;
        }
        return true;
    }
