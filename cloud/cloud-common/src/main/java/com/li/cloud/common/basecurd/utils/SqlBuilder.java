...
public class SqlBuilder {
     ...
     * @date: 2019-10-02
     */
    public <T> String deleteBatch(Class<T> entityClass, String deleteField, String deleteVals){
        tableName = entityUtil.getTableName(entityClass);
        col = entityUtil.getDeleteBatch(entityClass, deleteField, deleteVals); // 获取需要删除的id条件
        SQL sql = new SQL(){
            {
                DELETE_FROM(tableName).WHERE(col[0]);
            }
