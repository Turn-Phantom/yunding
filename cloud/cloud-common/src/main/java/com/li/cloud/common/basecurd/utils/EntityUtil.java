package com.li.cloud.common.basecurd.utils;

import com.li.cloud.common.annotations.ExcludeField;
import com.li.cloud.common.basecurd.config.ConfigParam;
import com.li.cloud.common.utils.MathZeroUtil;
import com.li.cloud.common.utils.enumutils.SpecialDataType;
...
public class EntityUtil {
   ...
   public <T> String getTableName(T entity){
        ...
        if(ObjectUtils.isEmpty(tabAnno) || StringUtils.isEmpty(tabAnno.name())){
    ...
    public <T> String getTableName(Class<T> entity){
        Table tabAnno = entity.getAnnotation(Table.class);
        if(ObjectUtils.isEmpty(tabAnno) || StringUtils.isEmpty(tabAnno.name())){
            throw new RuntimeException("获取数据库表名失败；若未对实体指定表名注解，需添加@Table注解，" +
                    "若未指定表名，则需要指定数据库对应的表名；例如：@Table(name = \"table_name\")");
        }
     ...
     <T> String[] getInsertColumn(T entity) {
        ...
        try {
            for (Field field : fields) {
                ...
                // 若没有@Column注解，则不处理字段
                if(ObjectUtils.isEmpty(colAnno)){
                    continue;
                }
                ...
                if(!ObjectUtils.isEmpty(idAnno)){
                    // 判断数据库类型; 默认为mysql类型；
                    if(StringUtils.isEmpty(databaseType) || "MYSQL".equals(databaseType.toUpperCase())){
                        ...
                        if(!ObjectUtils.isEmpty(sqAnno) && !StringUtils.isEmpty(sqAnno.generator())){
            ...
            String columnKey = col.deleteCharAt(col.length() - 1).toString();
            String columnValue = colVal.deleteCharAt(colVal.length() - 1).toString();
            if(StringUtils.isEmpty(columnKey.replaceAll("\"", "")) || StringUtils.isEmpty(columnValue.replaceAll("\"", ""))){
    ...
    private String getJdbcType(Field field) {
        ...
        if(fieldType.equals(SpecialDataType.Integer.getTypeVal())){ // 整型
            ...
            if(!ObjectUtils.isEmpty(dateAnno)){ // 注解非空判断
                String pattern = dateAnno.pattern();
                if(!StringUtils.isEmpty(pattern) && "TIMESTAMP".equals(pattern.toUpperCase())){
                    jdbcType = "TIMESTAMP";
                } else{
                    jdbcType = "DATE";
                }
    ...
    private <T> boolean fieldValIsNull(T entity, Field field, boolean isIncludeNum){
        ...
        switch (fieldType){
            ...
            case "double":
                ...
                break;
            case "char":
                if(CharUtils.NUL == (char)fieldVal){
                    return true;
                }
                break;
            default:
     ...
     */
     <T> String[] getUpdateColumn(T entity, Boolean isUpdateAll) {
         ...
         for (Field field : fields) {
             ...
             if(!ObjectUtils.isEmpty(excludeField)){
                 continue;
             }
             Column colAnno = field.getAnnotation(Column.class);
             if(ObjectUtils.isEmpty(colAnno)){
         ...
         String setField = sbField.deleteCharAt(sbField.length() - 1).toString();
         // 更新值空处理
         if(StringUtils.isEmpty(setField.replaceAll("\"", ""))){
     ...
     <T> String[] getSelectByIdCol(T entity) {
         ...
         String columnVal = sbField.deleteCharAt(sbField.length() - 1).toString();
         String condStr = sbCondition.toString();
         // 对sql字段片段和条件片段进行非空处理
         if(StringUtils.isEmpty(columnVal.replaceAll("\"",""))){
             throw new RuntimeException("根据id查询数据失败：查询字段未检测到@Column注解");
         }
         if(StringUtils.isEmpty(condStr.replaceAll("\"", ""))){
    ...
    public <T> String[] getDeleteIds(T entity) {
        ...
        for (Field field : fields) {
            ...
            // 查找含有@Id注解的字段
            Id idAnno = field.getAnnotation(Id.class);
            if(ObjectUtils.isEmpty(idAnno) || ObjectUtils.isEmpty(colAnno)){
                continue;
            }
            if(hasId){
            ...
            if(isStringType(field)){
                ...
                idCondition = idCondition.deleteCharAt(idCondition.length() - 1);
                idCondition.append(")");
            } else{
    ...
    /**
     * @desc: 根据指定字段批量删除数据（指定字段指定值删除）; 默认以逗号分割
     * @param: entity 实体对象
     ...
     * @date: 2019-10-02
     */
    public <T> String[] getDeleteBatch(Class<T> entityClass, String deleteField, String deleteVals) {
        String []val = new String[2];
        ...
        for (Field field : fields) {
            ...
            deleteCondition = deleteCondition.deleteCharAt(deleteCondition.length() - 1);
    ...
    public <T> String[] getDeleteByField(T entity, String deleteField) {
        ...
        for (Field field : fields) {
            ...
            if(isStringType(field)){
                ...
                sbCondition = sbCondition.deleteCharAt(sbCondition.length() - 1);
                sbCondition.append(")");
            }else{
    ...
    public <T> String[] getSelectByFieldCol(T entity, String queryField) {
        ...
        for (Field field : fields) {
            ...
            if(queryField.equals(fieldName)){
                sbCondition.append(colName).append("=#{").append("arg0.").append(queryField).append("}");
            }
            sbField.append(colName).append(" as ").append(fieldName);
            sbField.append(",");
        }
